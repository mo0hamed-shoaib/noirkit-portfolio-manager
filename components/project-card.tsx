...
export function ProjectCard({ project, techStackIcons }: ProjectCardProps) {
  ...
  const getTechIcon = (techName: string) => {
    const tech = techStackIcons.find((t) => t.name.toLowerCase() === techName.toLowerCase())
    return tech?.icon
  }
  ...
  return (
    ...
    >
        ...
        {project.images.length > 1 && (
          <>
            <button
              ...
              className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/70 hover:bg-black/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 backdrop-blur-sm border border-white/20"
            >
            ...
            <button
              onClick={nextImage}
              title="Next image"
              className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/70 hover:bg-black/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 backdrop-blur-sm border border-white/20"
            >
      ...
      <div
        className={`absolute inset-0 bg-black/95 backdrop-blur-sm flex flex-col justify-center items-center p-4 transition-opacity duration-300 z-20 ${
          isHovered ? "opacity-100" : "opacity-0"
        }`}
      >
        <h3 className="text-xl font-bold text-white mb-2 text-center">{project.name}</h3>
        <p className="text-gray-300 text-sm mb-4 text-center line-clamp-3 leading-relaxed">{project.description}</p>
        ...
        <div className="flex flex-wrap gap-2 mb-4 justify-center">
          {project.techStack.map((tech, index) => {
            ...
            return (
              <div key={index} className="w-8 h-8 text-white/80 hover:text-white transition-colors" title={tech}>
                {iconPath ? (
        ...
        <div className="flex gap-3">
          ...
          {project.githubLink && (
            <CustomButton variant="outline" size="sm" asChild className="shadow-lg">
