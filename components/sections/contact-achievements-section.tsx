...
import { Star, GraduationCap, Award, Mail, ArrowRight, Clock, Calendar, MessageSquare, ExternalLink, Coffee, Moon, Gamepad2, Trophy, MessageCircle } from "lucide-react"
...
export function ContactAchievementsSection({ achievements, onContactClick }: ContactAchievementsSectionProps) {
  ...
  useEffect(() => {
    if (achievements.length > maxVisibleAchievements) {
      const interval = setInterval(() => {
        ...
        setTimeout(() => {
          setCurrentAchievementIndex((prev) => (prev + maxVisibleAchievements >= achievements.length ? 0 : prev + 1))
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <section className="border border-white/20 rounded-xl p-4 sm:p-6 bg-gradient-to-br from-gray-900/30 to-black/30 backdrop-blur-sm group hover:border-white/30 transition-all duration-300 relative overflow-hidden min-h-[400px] flex flex-col">
        ...
        <div className="flex items-start justify-between mb-4 sm:mb-6">
          <div className="flex-1 min-w-0">
            <h2 className="text-lg sm:text-xl font-mono text-white font-bold mb-2">Let's Connect</h2>
            <div className="flex items-center gap-1 mt-1">
            <MessageCircle className="w-4 h-4 text-pink-400" />
              <span className="text-sm text-gray-400">Ready to bring your ideas to life</span>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col space-y-4 sm:space-y-6">
          ...
          {/* Services Grid */}
          <div className="space-y-3">
            <div className="text-xs sm:text-sm text-gray-500 font-medium">Services & Expertise</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div className="flex items-center gap-2 text-xs sm:text-sm">
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full flex-shrink-0"></div>
                <span className="text-gray-400 truncate">Full-Stack Development</span>
              </div>
              <div className="flex items-center gap-2 text-xs sm:text-sm">
              ...
              <div className="flex items-center gap-2 text-xs sm:text-sm">
                ...
                <span className="text-gray-400 truncate">Branding & Strategy</span>
              </div>
            </div>
          </div>
          ...
          {/* Fun Facts */}
          <div className="space-y-3">
            <div className="text-xs sm:text-sm text-gray-500 font-medium">Fun Facts</div>
        ...
        <div className="mt-4 sm:mt-6 pt-4 border-t border-white/10">
          <CustomButton 
            ...
            className="w-full sm:w-auto px-4 sm:px-6 group/button justify-center sm:justify-start"
          >
            Get in Touch
      ...
      <section className="border border-white/20 rounded-xl p-4 sm:p-6 bg-gradient-to-br from-gray-900/30 to-black/30 backdrop-blur-sm relative overflow-hidden min-h-[400px] flex flex-col group hover:border-white/30 transition-all duration-300">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <div className="flex-1 min-w-0">
            <h2 className="text-lg sm:text-xl font-mono text-white font-bold">Achievements & Education</h2>
                          <div className="flex items-center gap-1 mt-1">
                <Trophy className="w-4 h-4 text-yellow-400" />
                <span className="text-sm text-gray-400">Professional milestones & academic excellence</span>
              </div>
          </div>
        </div>
        ...
        {achievements.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center space-y-4">
            ...
            <div className="text-center">
              <p className="text-gray-400 text-sm mb-2">No achievements added yet</p>
              <p className="text-gray-500 text-xs">Your accomplishments will appear here</p>
            </div>
          </div>
        ) : (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-gray-400 text-xs">
                    {achievements.filter(a => a.type === 'education').length} Education • {achievements.filter(a => a.type === 'achievement').length} Awards
                  </div>
                </div>
              </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <h4 className="font-medium text-sm text-white line-clamp-1">{achievement.title}</h4>
                          {achievement.link && (
                            <ExternalLink className="w-3 h-3 text-gray-500 hover:text-gray-300 cursor-pointer flex-shrink-0" />
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          >
                            {achievement.type === "education" ? "Education" : "Achievement"}
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            {achievements.length > maxVisibleAchievements && (
              <div className="h-[60px] flex items-center justify-between mt-4 pt-4 border-t border-white/10 flex-shrink-0">
                <div className="flex justify-center flex-1 gap-1">
                  {Array.from({ length: Math.ceil(achievements.length / maxVisibleAchievements) }, (_, index) => (
                    <button
                      key={index}
                      onClick={() => handlePageChange(index)}
                      className={`w-2 h-2 rounded-full transition-all duration-200 ${
                        Math.floor(currentAchievementIndex / maxVisibleAchievements) === index 
                          ? "bg-white w-4" 
                ...
                <div className="text-xs text-gray-500 ml-4">
                  {currentAchievementIndex + 1}-{Math.min(currentAchievementIndex + maxVisibleAchievements, achievements.length)} of {achievements.length}
                </div>
              </div>
            )}
          </div>
