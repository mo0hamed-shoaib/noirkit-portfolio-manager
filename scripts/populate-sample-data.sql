-- IMPORTANT: Replace 'YOUR_USER_ID_HERE' with your actual user ID from Supabase Auth
-- You can find your user ID in the Supabase Dashboard under Authentication > Users

-- Insert personal info for the user
INSERT INTO profiles (
  id,
  email,
  name,
  job_title,
  bio,
  profile_image,
  phone,
  location,
  cv_file
) VALUES (
  'YOUR_USER_ID_HERE',
  'john.doe@example.com',
  'John Doe',
  'Full Stack Developer',
  'Passionate developer with 5+ years of experience building modern web applications. I love creating efficient, scalable solutions and learning new technologies.',
  '/placeholder.svg?height=200&width=200',
  '+1 (555) 123-4567',
  'San Francisco, CA',
  null
) ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
  job_title = EXCLUDED.job_title,
  bio = EXCLUDED.bio,
  profile_image = EXCLUDED.profile_image,
  email = EXCLUDED.email,
  phone = EXCLUDED.phone,
  location = EXCLUDED.location;

-- Insert sample projects
INSERT INTO projects (
  user_id,
  name,
  description,
  deploy_link,
  github_link,
  tech_stack,
  images,
  order_index
) VALUES 
(
  'YOUR_USER_ID_HERE',
  'E-Commerce Platform',
  'A full-featured e-commerce platform with user authentication, payment processing, and admin dashboard.',
  'https://ecommerce-demo.vercel.app',
  'https://github.com/johndoe/ecommerce-platform',
  ARRAY['React', 'Next.js', 'TypeScript', 'Stripe', 'PostgreSQL'],
  ARRAY['/placeholder.svg?height=400&width=600&text=E-Commerce+Platform'],
  0
),
(
  'YOUR_USER_ID_HERE',
  'Task Management App',
  'A collaborative task management application with real-time updates and team collaboration features.',
  'https://taskmanager-demo.vercel.app',
  'https://github.com/johndoe/task-manager',
  ARRAY['React', 'Node.js', 'Socket.io', 'MongoDB'],
  ARRAY['/placeholder.svg?height=400&width=600&text=Task+Manager'],
  1
),
(
  'YOUR_USER_ID_HERE',
  'Weather Dashboard',
  'A beautiful weather dashboard with location-based forecasts and interactive charts.',
  'https://weather-dashboard-demo.vercel.app',
  'https://github.com/johndoe/weather-dashboard',
  ARRAY['Vue.js', 'Chart.js', 'OpenWeather API'],
  ARRAY['/placeholder.svg?height=400&width=600&text=Weather+Dashboard'],
  2
);

-- Insert sample tech stack
INSERT INTO tech_stack (
  user_id,
  name,
  icon,
  order_index
) VALUES 
(
  'YOUR_USER_ID_HERE',
  'React',
  'M14.23 12.004a2.236 2.236 0 0 1-2.235 2.236 2.236 2.236 0 0 1-2.236-2.236 2.236 2.236 0 0 1 2.235-2.236 2.236 2.236 0 0 1 2.236 2.236zm2.648-10.69c-1.346 0-3.107.96-4.888 2.622-1.78-1.653-3.542-2.602-4.887-2.602-.41 0-.783.093-1.106.278-1.375.793-1.683 3.264-.973 6.365C1.98 8.917 0 10.42 0 12.004c0 1.59 1.99 3.097 5.043 4.03-.704 3.113-.39 5.588.988 6.38.32.187.69.275 1.102.275 1.345 0 3.107-.96 4.888-2.624 1.78 1.654 3.542 2.603 4.887 2.603.41 0 .783-.09 1.106-.275 1.374-.792 1.683-3.263.973-6.365C22.02 15.096 24 13.59 24 12.004c0-1.59-1.99-3.097-5.043-4.032.704-3.11.39-5.587-.988-6.38-.318-.184-.688-.277-1.092-.278zm-.005 1.09v.006c.225 0 .406.044.558.127.666.382.955 1.835.73 3.704-.054.46-.142.945-.25 1.44-.96-.236-2.006-.417-3.107-.534-.66-.905-1.345-1.727-2.035-2.447 1.592-1.48 3.087-2.292 4.105-2.295zm-9.77.02c1.012 0 2.514.808 4.11 2.28-.686.72-1.37 1.537-2.02 2.442-1.107.117-2.154.298-3.113.538-.112-.49-.195-.964-.254-1.42-.23-1.868.054-3.32.714-3.707.19-.09.4-.127.563-.132zm4.882 3.05c.455.468.91.992 1.36 1.564-.44-.02-.89-.034-1.36-.034-.47 0-.92.014-1.36.034.44-.572.895-1.096 1.36-1.564zM12 8.1c.74 0 1.477.034 2.202.093.406.582.802 1.203 1.183 1.86.372.64.71 1.29 1.018 1.946-.308.655-.646 1.31-1.013 1.95-.38.66-.773 1.288-1.18 1.87-.728.063-1.466.098-2.21.098-.74 0-1.477-.035-2.202-.093-.406-.582-.802-1.204-1.183-1.86-.372-.64-.71-1.29-1.018-1.946.303-.657.646-1.313 1.013-1.954.38-.66.773-1.286 1.18-1.868.728-.064 1.466-.098 2.21-.098zm-3.635.254c-.24.377-.48.763-.704 1.16-.225.39-.435.782-.635 1.174-.265-.656-.49-1.31-.676-1.947.64-.15 1.315-.283 2.015-.386zm7.26 0c.695.103 1.365.23 2.006.387-.18.632-.405 1.282-.66 1.933-.2-.39-.41-.783-.64-1.174-.225-.392-.465-.774-.705-1.146zm3.063.675c.484.15.944.317 1.375.498 1.732.74 2.852 1.708 2.852 2.476-.005.768-1.125 1.74-2.857 2.475-.42.18-.88.342-1.355.493-.28-.958-.646-1.956-1.1-2.98.45-1.017.81-2.01 1.085-2.964zm-13.395.004c.278.96.645 1.957 1.1 2.98-.45 1.017-.812 2.01-1.086 2.964-.484-.15-.944-.318-1.37-.5-1.732-.737-2.852-1.706-2.852-2.474 0-.768 1.12-1.742 2.852-2.476.42-.18.88-.342 1.356-.494zm11.678 4.28c.265.657.49 1.312.676 1.948-.64.157-1.316.29-2.016.39.24-.375.48-.762.705-1.158.225-.39.435-.788.636-1.18zm-9.945.02c.2.392.41.783.64 1.175.23.39.465.772.705 1.143-.695-.102-1.365-.23-2.006-.386.18-.63.406-1.282.66-1.933zM17.92 16.32c.112.493.2.968.254 1.423.23 1.868-.054 3.32-.714 3.708-.147.09-.338.128-.563.128-1.012 0-2.514-.807-4.11-2.28.686-.72 1.37-1.536 2.02-2.44 1.107-.118 2.154-.3 3.113-.54zm-11.83.01c.96.234 2.006.415 3.107.532.66.905 1.345 1.727 2.035 2.446-1.595 1.483-3.092 2.295-4.11 2.295-.22-.005-.406-.05-.553-.132-.666-.38-.955-1.834-.73-3.703.054-.46.142-.944.25-1.438zm4.56.64c.44.02.89.034 1.36.034.47 0 .92-.014 1.36-.034-.44.572-.895 1.095-1.36 1.56-.465-.467-.92-.992-1.36-1.56z',
  0
),
(
  'YOUR_USER_ID_HERE',
  'TypeScript',
  'M1.125 0C.502 0 0 .502 0 1.125v21.75C0 23.498.502 24 1.125 24h21.75c.623 0 1.125-.502 1.125-1.125V1.125C24 .502 23.498 0 22.875 0zm17.363 9.75c.612 0 1.154.037 1.627.111a6.38 6.38 0 0 1 1.306.34v2.458a3.95 3.95 0 0 0-.643-.361 5.093 5.093 0 0 0-.717-.26 5.453 5.453 0 0 0-1.426-.2c-.3 0-.573.028-.819.086a2.1 2.1 0 0 0-.623.242c-.17.104-.3.229-.393.374a.888.888 0 0 0-.14.49c0 .196.053.373.156.529.104.156.252.304.443.444s.423.276.696.41c.273.135.582.274.926.416.47.197.892.407 1.266.628.374.222.695.473.963.753.268.279.472.598.614.957.142.359.214.776.214 1.253 0 .657-.125 1.21-.373 1.656a3.033 3.033 0 0 1-1.012 1.085 4.38 4.38 0 0 1-1.487.596c-.566.12-1.163.18-1.79.18a9.916 9.916 0 0 1-1.84-.164 5.544 5.544 0 0 1-1.512-.493v-2.63a5.033 5.033 0 0 0 3.237 1.2c.333 0 .624-.03.872-.09.249-.06.456-.144.623-.25.166-.108.29-.234.373-.38a1.023 1.023 0 0 0-.074-1.089 2.12 2.12 0 0 0-.537-.5 5.597 5.597 0 0 0-.807-.444 27.72 27.72 0 0 0-1.007-.436c-.918-.383-1.602-.852-2.053-1.405-.45-.553-.676-1.222-.676-2.005 0-.614.123-1.141.369-1.582.246-.441.58-.804 1.004-1.089a4.494 4.494 0 0 1 1.47-.629 7.536 7.536 0 0 1 1.77-.201zm-15.113.188h9.563v2.166H9.506v9.646H6.789v-9.646H3.375z',
  1
),
(
  'YOUR_USER_ID_HERE',
  'Node.js',
  'M11.435 1.587l-8.201 4.735c-.292.168-.473.48-.473.817v9.721c0 .337.181.649.473.817l8.201 4.735c.292.168.649.168.941 0l8.201-4.735c.292-.168.473-.48.473-.817V7.139c0-.337-.181-.649-.473-.817L12.376 1.587c-.292-.168-.649-.168-.941 0zM12 2.542l7.333 4.235v8.446L12 19.458l-7.333-4.235V6.777L12 2.542zm-1.732 4.301c-2.753 0-4.429 1.188-4.429 3.139 0 2.069 1.676 2.91 4.429 2.91h.346c1.387 0 2.078.462 2.078 1.386 0 .924-.691 1.386-2.078 1.386h-.346c-1.387 0-2.078-.462-2.078-1.386H6.458c0 1.951 1.676 3.139 4.429 3.139h.346c2.753 0 4.429-1.188 4.429-3.139 0-2.069-1.676-2.91-4.429-2.91h-.346c-1.387 0-2.078-.462-2.078-1.386 0-.924.691-1.386 2.078-1.386h.346c1.387 0 2.078.462 2.078 1.386h1.732c0-1.951-1.676-3.139-4.429-3.139h-.346z',
  2
);

-- Insert sample achievements
INSERT INTO achievements (
  user_id,
  title,
  description,
  date,
  type,
  order_index
) VALUES 
(
  'YOUR_USER_ID_HERE',
  'Bachelor of Computer Science',
  'Graduated with honors from University of California, Berkeley. Specialized in software engineering and web development.',
  '2019',
  'education',
  0
),
(
  'YOUR_USER_ID_HERE',
  'AWS Certified Developer',
  'Achieved AWS Certified Developer - Associate certification, demonstrating expertise in developing and maintaining applications on AWS.',
  'March 2023',
  'achievement',
  1
),
(
  'YOUR_USER_ID_HERE',
  'Hackathon Winner',
  'Won first place at TechCrunch Disrupt Hackathon 2022 for developing an innovative AI-powered productivity tool.',
  'October 2022',
  'achievement',
  2
);

-- Insert sample social links
INSERT INTO social_links (
  user_id,
  platform,
  url,
  icon,
  order_index
) VALUES 
(
  'YOUR_USER_ID_HERE',
  'GitHub',
  'https://github.com/johndoe',
  'M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z',
  0
),
(
  'YOUR_USER_ID_HERE',
  'LinkedIn',
  'https://linkedin.com/in/johndoe',
  'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z',
  1
),
(
  'YOUR_USER_ID_HERE',
  'Twitter',
  'https://twitter.com/johndoe',
  'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z',
  2
);

-- Insert sample contact form
INSERT INTO contact_form (
  user_id,
  title,
  description,
  show_contact_info
) VALUES (
  'YOUR_USER_ID_HERE',
  'Get In Touch',
  'Let''s discuss your next project and bring your ideas to life.',
  true
) ON CONFLICT (user_id) DO UPDATE SET
  title = EXCLUDED.title,
  description = EXCLUDED.description,
  show_contact_info = EXCLUDED.show_contact_info;

-- Get the contact form ID for inserting fields
DO $$
DECLARE
    form_id UUID;
BEGIN
    SELECT id INTO form_id FROM contact_form WHERE user_id = 'YOUR_USER_ID_HERE';
    
    -- Insert sample contact form fields
    INSERT INTO contact_form_fields (
      contact_form_id,
      name,
      label,
      type,
      required,
      placeholder,
      order_index
    ) VALUES 
    (
      form_id,
      'name',
      'Your Name',
      'text',
      true,
      'Enter your full name',
      0
    ),
    (
      form_id,
      'email',
      'Email Address',
      'email',
      true,
      'your@email.com',
      1
    ),
    (
      form_id,
      'subject',
      'Subject',
      'text',
      true,
      'What is this about?',
      2
    ),
    (
      form_id,
      'message',
      'Message',
      'textarea',
      true,
      'Tell me about your project...',
      3
    );
END $$;
