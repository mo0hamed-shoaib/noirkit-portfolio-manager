...
import { Plus, Edit, Trash2, Award, GraduationCap, Calendar, Save } from "lucide-react"
...
export default function AchievementsPage() {
  ...
  const educationItems = sortedAchievements.filter((item) => item.type === "education")
  const achievementItems = sortedAchievements.filter((item) => item.type === "achievement")
    ...
    >
      <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full ${
                  achievement.type === "education" ? "bg-blue-400/20 text-blue-400" : "bg-yellow-400/20 text-yellow-400"
                }`}
              >
                {achievement.type}
              </span>
        ...
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <CustomButton variant="ghost" size="sm" onClick={() => openModal(achievement)}>
            <Edit className="w-4 h-4" />
          </CustomButton>
          <CustomButton variant="ghost" size="sm" onClick={() => handleDelete(achievement.id)}>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-mono mb-2">Achievements & Education</h1>
          <p className="text-gray-400">Manage your accomplishments and educational background</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-sm text-gray-500 bg-white/5 px-2 py-1 rounded">{educationItems.length} items</span>
          </div>
          <div className="space-y-4">
            {educationItems.length === 0 ? (
              <div className="border border-white/20 rounded-xl p-8 text-center bg-gray-900/30">
                <GraduationCap className="w-8 h-8 text-gray-500 mx-auto mb-3" />
                <p className="text-gray-500 text-sm mb-4">No education entries yet</p>
                <CustomButton
                  size="sm"
                  onClick={() => {
        ...
        <div>
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-sm text-gray-500 bg-white/5 px-2 py-1 rounded">{achievementItems.length} items</span>
          </div>
          <div className="space-y-4">
            {achievementItems.length === 0 ? (
              <div className="border border-white/20 rounded-xl p-8 text-center bg-gray-900/30">
                <Award className="w-8 h-8 text-gray-500 mx-auto mb-3" />
                <p className="text-gray-500 text-sm mb-4">No achievements yet</p>
                <CustomButton
                  size="sm"
                  onClick={() => {
        ...
        <DialogContent className="max-w-2xl bg-black border border-white text-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-black border border-white">
                  ...
                  <SelectItem value="achievement" className="text-white hover:bg-white/10">
                    <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <div className={`absolute bottom-2 right-2 text-xs ${
                  formData.description.length > 140 ? 'text-red-400' : 
                  formData.description.length > 120 ? 'text-yellow-400' : 'text-gray-500'
                }`}>
                  {formData.description.length}/140
                </div>
              </div>
              <p className="text-xs text-gray-500">
                Keep your description concise and impactful. This helps maintain consistent display across your portfolio.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-gray-500">
                You can use any format: year only (2023), month/year (June 2023), or range (2020-2024)
              </p>
            </div>

            <div className="flex gap-4">
              <CustomButton type="submit" disabled={formData.description.length > 140}>
